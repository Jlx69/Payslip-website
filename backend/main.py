...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["https://jlx69.github.io", "http://127.0.0.1:5500", "http://localhost:5500", "null", "*"],
...
# ── Detect company from employee ID ────────────────────────────────
...
def download_payslip(
    ...
    payslip_data = {
        ...
        "paid_days": payslip.paid_days, "basic": payslip.basic,
        "bonus": payslip.bonus, "shift_allowance": payslip.shift_allowance,
        "incentive": payslip.incentive, "lww": payslip.lww,
        "pf": payslip.pf, "esic": payslip.esic,
        "lwf": payslip.lwf, "transport": payslip.transport,
...
async def upload_wagesheet(
    ...
    def safe_date(val):
        """Convert date to readable string like 01-Apr-2025."""
        try:
            if pd.isna(val): return None
            return pd.to_datetime(val).strftime("%d-%b-%Y")
        except: return safe_str(val)
    ...
    for _, row in df.iterrows():
        try:
            ...
            doj_val     = safe_date(row.get("DOJ"))  # Column D
            ...
            if not employee:
                default_password = bcrypt.hashpw(emp_code.encode(), bcrypt.gensalt()).decode()
                employee = Employee(
                    employee_id=emp_code, name=emp_name,
                    department=safe_str(row.get("Department Name", "")) or "",
                    designation="", email="",
                    password_hash=default_password,
                    esi_no=None, uan=None,
                    company=emp_company, doj=doj_val,
                )
                db.add(employee)
            else:
                employee.name       = emp_name
                employee.department = safe_str(row.get("Department Name", "")) or ""
                employee.company    = emp_company
                employee.doj        = doj_val  # update DOJ on re-upload
            ...
            if existing_slip:
                existing_slip.paid_days=paid_days; existing_slip.basic=basic
                existing_slip.bonus=bonus; existing_slip.shift_allowance=shift_allowance
                existing_slip.incentive=incentive; existing_slip.lww=lww
                existing_slip.pf=pf; existing_slip.esic=esic
                existing_slip.lwf=lwf; existing_slip.transport=transport
                ...
                db.add(Payslip(
                    ...
                    company=emp_company, paid_days=paid_days,
                    basic=basic, bonus=bonus, shift_allowance=shift_allowance,
                    incentive=incentive, lww=lww, pf=pf, esic=esic,
                    lwf=lwf, transport=transport, gross_salary=gross_salary,
                    total_deduction=total_deduction, net_salary=net_salary,
                ))
                inserted += 1
...
async def upload_uan_esic(file: UploadFile = File(...), db: Session = Depends(get_db)):
    contents = await file.read()
    try:
    ...
    for _, row in df.iterrows():
        try:
            ...
            if not employee: not_found.append(emp_id); continue
